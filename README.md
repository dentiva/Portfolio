...
└──────────┬───────────┘
           ▼
┌──────────────────────┐
│ TradingView scrape   │  ← static HTML, no JS rendering — sole live source
│ + SQLite cache       │    cached prices when the live scrape fails
└──────────┬───────────┘
           ▼
┌──────────────────────┐
...
(the canonical SQLite lookup key — kept named `yf_symbol` for backwards
compat with existing data; treat it as just a unique identifier). The
router does this for every poll:

1. **TradingView** for any symbol with `tv_url` set.
...
2. **SQLite cache** as last resort. The dashboard shows price source per
   row so you can see which holdings are live vs stale.

If TradingView blocks the IP or changes its schema, every symbol falls
back to cache; once the cache ages out the dashboard shows `—` and the
poll reports `missing`. Macro indicators on FX/commodity/index pages
(USD/INR, oil, gold, VIX, S&P, DXY) don't expose price through the FAQ
schema and may go dark unless their TV page format changes — they were
previously served by yfinance, which has been removed.
...
### Swap price source
The router in `app/prices.py` uses TradingView as the only live source and
SQLite as the last-resort cache. To disable a specific holding from
fetching, set `tv_url: null` in `portfolio.json` — that holding will read
straight from cache (and go `missing` once the cache ages out).
...
the embedded FAQ schema so they will fall through to cache/missing.

If TradingView fails for too long, alternatives that need only a
`prices.py` rewrite:
...
- **TradingView is unofficial.** It can change page structure or block IPs
  with no notice. Falls back to cached prices when scraping fails; the
  dashboard flags stale data and the poll logs `cached`/`missing` counts.
...
If TradingView returns nothing for a symbol persistently (the page format
changed, the IP got blocked, or the symbol's TV page never had the FAQ
schema in the first place), either wait for an upstream fix, swap the
price provider in `prices.py`, or temporarily hardcode in `prices.py`.
